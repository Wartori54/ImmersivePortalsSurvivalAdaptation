...
public class PortalCompleter extends Item {
    ...
    public static class Data {
        ...
        public static NbtCompound serialize(boolean[] boolArray) {
            NbtCompound tag = new NbtCompound();
            tag.putIntArray("portalsToComplete", Utils.boolArray2IntArray(boolArray));
            return tag;
        }

        public static void deserialize(NbtCompound tag) {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient()) {
            ...
            if (portal != null) {
                ...
                Data.deserialize(user.getStackInHand(hand).getOrCreateNbt());
    ...
    public void appendStacks(ItemGroup group, DefaultedList<ItemStack> stacks) {
        if (this.isIn(group)) {
            for (boolean i :
                ...
                NbtCompound tag = Data.serialize(new boolean[]{true, i, i});
