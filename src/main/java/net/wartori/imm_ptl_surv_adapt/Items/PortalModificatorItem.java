...
public class PortalModificatorItem extends Item {
    ...
    public void appendStacks(ItemGroup group, DefaultedList<ItemStack> stacks) {
        if (this.isIn(group)) {
            ...
            moveHalfStack.setNbt(moveHalfStackData.serialize());
            ...
            moveMinusHalfStack.setNbt(moveMinusHalfStackData.serialize());
            ...
            rotate15Stack.setNbt(rotate15StackData.serialize());
            ...
            rotate90Stack.setNbt(rotate90StackData.serialize());
            ...
            deleteStack.setNbt(deleteStackData.serialize());
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        Data data = Data.deserialize(user.getStackInHand(hand).getOrCreateNbt());
        if (!world.isClient()) {
...
//                    AtomicInteger portalsRotated = new AtomicInteger();
                    ...
                    PortalManipulation.getPortalClutter(world, portal.getDestPos(), portal.transformLocalVecNonScale(portal.getNormal()),  p -> Objects.equals(p.specificPlayerId, portal.specificPlayerId) && portal.getDiscriminator() != (p.getDiscriminator()))
                            .forEach(e -> {
                                ...
                                NbtCompound portalNbt1 = e.writeNbt(new NbtCompound());
                                portalNbt1.putBoolean("adjustPositionAfterTeleport", true);
                                e.readNbt(portalNbt1);
                                e.reloadAndSyncToClient();
                            });
...
//                                    System.out.println("was y");
                    ...
                    if (!user.isSneaking()) {
                        ...
                        if (facesToDeleteDecoded[3]) {
                            PortalManipulation.getPortalClutter(world, portal.getDestPos(), portal.transformLocalVecNonScale(portal.getNormal()), p -> Objects.equals(p.specificPlayerId, portal.specificPlayerId) && portal.getDiscriminator() != (p.getDiscriminator()))
                                    .forEach(p -> p.remove(Entity.RemovalReason.KILLED));
                        }
    ...
    public static Vec3f vec3ito3f(Vec3i vec) {
