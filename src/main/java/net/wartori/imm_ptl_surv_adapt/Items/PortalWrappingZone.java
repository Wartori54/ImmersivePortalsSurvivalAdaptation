...
//        }
    ...
    public void appendStacks(ItemGroup group, DefaultedList<ItemStack> stacks) {
        if (this.isIn(group) || RegisterItemGroups.IMMERSIVE_PORTALS_SURVIVAL_ADAPTATION_GROUP == group) {
            ...
            stackOut.setNbt(new Data(WarpingType.OUT, new BlockPos(0,0,0), new BlockPos(0,0,0), false, false).serialize());
            stacks.add(stackOut);

            ItemStack stackBoth = new ItemStack(this);
            stackBoth.setNbt(new Data(WarpingType.BOTH, new BlockPos(0,0,0), new BlockPos(0,0,0), false, false).serialize());
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!context.getWorld().isClient()) {
            ...
            if (context.getWorld().getBlockState(context.getBlockPos()).getBlock() == Register.WRAPPING_ZONE_START) {
                data = Data.deserialize(stack.getOrCreateNbt());
                data.startPos = context.getBlockPos();
                data.isValidStart = true;
                stack.setNbt(data.serialize());
                ...
                if (Data.deserialize(stack.getOrCreateNbt()).isValidStart && Data.deserialize(stack.getOrCreateNbt()).isValidEnd) {
    ...
    public static void createWarpingBox(WarpingType warpingType, BlockPos start, BlockPos end, ServerWorld world) {
        ...
        for (Direction direction:
            ...
            if (warpingType == WarpingType.IN) {
                ...
                PortalManipulation.getPortalClutter(world, portal.getOriginPos(), portal.getNormal(), p -> true).forEach(p -> p.remove(Entity.RemovalReason.KILLED));
                ...
                PortalManipulation.getPortalClutter(world, portal.getOriginPos(), portal.getNormal(), p -> true).forEach(p -> p.remove(Entity.RemovalReason.KILLED));
                PortalManipulation.getPortalClutter(world, portalOtherSide.getOriginPos(), portalOtherSide.getNormal(), p -> true).forEach(p -> p.remove(Entity.RemovalReason.KILLED));
